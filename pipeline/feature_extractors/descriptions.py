...
class FeatureExtractorDescriptionsFelix(FeatureExtractorBase):
    def __init__(self, path):
        self.path = path

    def extract(self, transactions: pd.DataFrame, stories: pd.DataFrame, users: pd.DataFrame, candidates: pd.DataFrame) -> pd.DataFrame:
        stories_features = pd.read_csv(self.path)

        candidates = candidates.merge(stories_features, on="story_id", how="left")
